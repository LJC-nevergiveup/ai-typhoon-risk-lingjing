...
- **人工核验**：作者信息已回填（李金澄，中国地震局兰州地震研究所，个人参赛）；正式访问 URL 待部署后回填、历史会话截图待人工补录、素材许可终审；本文档所写科学数值均与 `docs/competition/data-provenance.md`、各 manifest / sources.json 核对一致，未改动任何数值。
- **发现问题**：正式部署 URL 尚缺；历史 Codex 会话截图需从真实会话补录。
- **最终处理**：作者信息已回填；以占位标注仍待补项（正式 URL / 截图），**不编造 URL、截图内容与素材许可**。
...
| 7 | 文档数值与 provenance / manifest 核对；作者信息回填（李金澄） | 数值一致 |
